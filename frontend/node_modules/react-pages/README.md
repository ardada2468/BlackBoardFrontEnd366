...
Suppose there's a "forum" web applications having `<Thread/>` pages with URLs like `/thread/:id`, and one thread could link to another thread. When a user navigates to a thread and clicks a link to another thread there, route transition will start: the "previous" thread page will still be rendered while the "next" thread thread page is being loaded. The issue is that both these pages use the same Redux reducer, and so, as the "next" thread is being loaded, the "previous" thread page is gonna be updated with the updated Redux state. In simple cases, that would not be an issue. In complex cases though, when a `<Thread/>` page has a state, it could result in data consistency bugs. For example, if a `<Thread/>` page had a `showCommentsFrom` index state variable, then, when it's rerendered with the data for the "next" thread, the `showCommentsFrom` index might go out of bounds breaking the page. To prevent such issues, all routes potentially linking to self should be rendered with a `key` corresponding to URL parameters. For example:

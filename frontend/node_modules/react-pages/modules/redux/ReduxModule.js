...
import { eventName, underscoredToCamelCase, DEFAULT_REDUX_EVENT_NAMING, generateReduxNamespace, generateReduxEventName } from './naming';
import { RESULT_ACTION_PROPERTY, ERROR_ACTION_PROPERTY } from './middleware/asynchronous'; // Sometimes modules for one project are imported from another project directory.
...
function () {
  function ReduxModule() {
    var namespace = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : generateReduxNamespace(counter.next());
...
function createAction(event, action, result, options, redux) {
  ...
  if (sync) {
    // Reducer
    redux.on(eventName(namespace, event), get_action_value_reducer(result)); // Redux "action creator"
...
function add_asynchronous_action_reducers(redux, namespace, event, result_reducer) {
  ...
  redux.on(eventName(namespace, pending_event_name), function (state) {
  ...
  redux.on(eventName(namespace, success_event_name), function (state, result) {
  ...
  redux.on(eventName(namespace, error_event_name), function (state, error) {

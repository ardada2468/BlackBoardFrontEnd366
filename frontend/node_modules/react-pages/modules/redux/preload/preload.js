...
import { redirect, goto as _goto, REDIRECT_ACTION_TYPE, GOTO_ACTION_TYPE, RedirectException } from '../../router';
...
export default function _preload(location, previousLocation, routerArgs, codeSplit, server, getCookie, getLocale, dispatch, getState) {
  ...
  var _isInstantTransition = !server && location.action === 'POP' && previousLocation && isInstantTransition(previousLocation, location); // Preload status object.
  ...
  var preloading = {}; // Can cancel previous preloading (on the client side)
  ...
  dispatch = instrumentDispatch(dispatch, server, preloading); // Preload all the required data for this route (page)

  var preload;

  if (!_isInstantTransition) {
    var _routes = routes;
    var _components = components; // Client-side optimization.
    ...
    preload = generatePreloadChain(preloaders, server, isInitialClientSidePreload, getState, dispatch, location, params, getCookie, preloading);
  ...
  if (getLocale) {
    loadTranslation = collectTranslations(components, routes, routeIndices, codeSplit, getLocale(getState()), dispatch);
  } // Combine promises.
  ...
  preloading.pending = true; // Preloading process cancellation

  preloading.cancel = function () {
    preloading.cancelled = true; // If `bluebird` is used,
  ...
  return promise.then( // Navigate to the new page
  function () {
    preloading.pending = false; // If this navigation process was cancelled
    ...
    preloading.pending = false; // May be a server-side special "redirect" error.
...
export function collectPreloadersFromComponents(components) {
  ...
  return components // Some wrapper routes can have no `component`.
...
function collectPreloadersFromRoutes(routes) {
  ...
  return routes.map(function (_) {
    ...
    return normalizeLoad(load).map(function (_ref2) {
      var load = _ref2.load,
          rest = _objectWithoutProperties(_ref2, ["load"]);
...
function filterByChangedRoutes(filtered, routes, routeParams) {
  ...
  if (window._react_pages_previous_routes) {
    var previous_routes = window._react_pages_previous_routes;
    var previous_routes_parameters = window._react_pages_previous_routes_parameters;
    var i = 0;

    while (i < routes.length - 1 && previous_routes[i] === routes[i] && isEqual(previous_routes_parameters[i], routeParams[i])) {
      i++;
    }

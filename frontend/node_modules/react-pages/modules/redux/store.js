...
import { convertRoutes, foundReducer, createRouterStoreEnhancers, initializeRouter } from '../router';
export default function _createStore(settings, data, createHistoryProtocol, httpClient, options) {
  ...
  var getData = createGetDataForPreload(codeSplit, server, onError, getLocale, getConvertedRoutes, getCookie);
  ...
  asynchronousMiddleware(httpClient, reduxEventNaming, server, http.onError, http.getErrorData));

  if (!server) {
    middleware.push(routerMiddleware(routes, codeSplit, onBeforeNavigate, onNavigate, stats, meta));
  ...
  storeEnhancers.push.apply(storeEnhancers, _toConsumableArray(createRouterStoreEnhancers(routes, createHistoryProtocol, {
    basename: settings.basename
  }))); // Redux middleware are applied in reverse order.
  // (which is counter-intuitive)

  storeEnhancers.push(applyMiddleware.apply(void 0, _toConsumableArray(middleware))); // Create Redux store.

  var store = getStoreEnhancersComposer(server, devtools).apply(void 0, storeEnhancers)(createStore)(createReducer(reducers, showLoadingInitially), data); // On the client side, add `hotReload()` function to the `store`.
  ...
  initializeRouter(store); // Return the Redux store
...
function createReducer(reducers, showLoadingInitially) {
  ...
  checkForAutogeneratedEventNameCollision(reducers); // Clone the object because it will be modified.

  reducers = _objectSpread({}, reducers); // Add `found` reducer.

  reducers.found = foundReducer; // Add `load` status reducer.

  reducers.preload = preloadReducer(showLoadingInitially); // // Add `@translate()` reducer.

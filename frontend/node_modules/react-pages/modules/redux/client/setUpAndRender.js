...
import { showInitialPreload, hideInitialPreload } from './initialPreload'; // This function is what's gonna be called from the project's code on the client-side.
//
...
export default function setUpAndRender(settings) {
  ...
  var httpClient = createHttpClient(settings, function () {
    return store;
  ...
  resetInstantNavigationChain(); // `showLoadingInitially` is handled in a special way
  ...
  return clientSideRender({
    ...
    if (error instanceof RedirectException) {
      ...
      store.dispatch(pushLocation(error.location)); // Reset all `react-pages` flags.

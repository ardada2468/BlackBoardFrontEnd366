...
function () {
  ...
  function HttpClient() {
    ...
    var getCookie = this.server ? function (name) {
      ...
      return cookies[name];
    } : _getCookie; // `superagent` doesn't save cookies by default on the server side.
    // Therefore calling `.agent()` explicitly to enable setting cookies.

    var agent = this.server ? superagent.agent() : superagent; // Define HTTP methods on this `http` utility instance
    ...
    var _loop = function _loop() {
      ...
      _this[method] = function (originalUrl, data) {
        ...
        var performHttpRequest = function performHttpRequest() {
          // Create Http request
          var request = new HttpRequest(method, url, data, {
            agent: agent,

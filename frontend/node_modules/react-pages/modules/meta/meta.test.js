import { updateMeta, generateMetaTagsMarkup, expandObjects, expandArrays, convertMeta, convertOpenGraphLocaleToLanguageTag } from './meta';
import TestDocument from './TestDocument';
describe("meta", function () {
  it("should update meta", function () {
    var document = new TestDocument([['charset', 'win1250'], ['og:locale', 'de'], ['og:locale', 'fr']]);
    updateMeta({
      title: 'Test',
  ...
  it("should transform \"siteName\" to \"site_name\"", function () {
    var document = new TestDocument([['charset', 'utf-8'], ['og:locale', 'en']]);
    updateMeta({
      siteName: 'Testing'
    }, document);
  ...
  it("should update charset", function () {
    var document = new TestDocument([['charset', 'win1250']]);
    updateMeta({
      charset: 'utf-8'
    }, document);
  ...
  it("should skip updating same charset", function () {
    var document = new TestDocument([['charset', 'utf-8']]);
    updateMeta({
      charset: 'utf-8'
    }, document);
    document.getMetaTags().should.deep.equal([['charset', 'utf-8']]);
  ...
  it("should generate meta tags markup with default title and charset", function () {
    generateMetaTagsMarkup({}).should.deep.equal(["<meta charset=\"utf-8\"/>", "<title></title>"]);
  });
  ...
  it('should expand arrays', function () {
    ...
    expandArrays(['key', ['subvalue1', 'subvalue2']]).should.deep.equal([['key', 'subvalue1'], ['key', 'subvalue2']]);
    expandArrays(['key', [{
      'subkey1': 'subvalue1',
      'subkey2': 'subvalue2'
    }, {
